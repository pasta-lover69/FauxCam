...
(function overrideGetUserMedia() {
  ...
  function requestImageFromExtension() {
    return new Promise((resolve) => {
      const responseHandler = (event) => {
        ...
        if (!event.data || event.data.type !== "VCAM_IMAGE_RESPONSE") return;
      ...
      window.postMessage({ type: "VCAM_REQUEST_IMAGE" }, "*");
  ...
  function loadImage(dataUrl) {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => reject(new Error("Failed to load virtual camera image."));
      img.src = dataUrl;
    });
  }
  ...
  navigator.mediaDevices.getUserMedia = async function virtualGetUserMedia(constraints) {
    ...
    try {
      ...
      console.info("[VirtualIDCamera] Serving virtual camera stream from uploaded image.");
      return virtualStream;
    } catch (error) {
      console.warn("[VirtualIDCamera] Failed to create virtual stream, falling back to real camera.", error);
      return originalGetUserMedia(constraints);
    }
  };
})();
